...
export function BrandReviews({ brandId, brandName, userId }: BrandReviewsProps) {
  ...
  const avgRatingNumber = Number(getAverageRating())
      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="text-5xl font-bold text-slate-900">{avgRatingNumber}</div>
            <div>
              <div className="flex gap-1 mb-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <svg
                    key={star}
                    className="w-5 h-5"
                    fill={parseFloat(avgRatingNumber) >= star ? '#FD8AE6' : '#E5E7EB'}
                    viewBox="0 0 20 20"
                  >
